#!/usr/bin/env python3
# PIERBITE | GLOS Probe v3 | 2026-08-20T19:40:00Z | v3
# Repo root filename (GitHub, prefix removed per D211): glos-probe-v3-2026-08-20-v3.py
# Local archive filename (Paul's machine):             Scripts_glos-probe-v3-2026-08-20-v3.py
#
# WHY v3 EXISTS: v2's live run proved something real and useful, not a bug:
#   (a) minimal single-variable query, last 6 days -> still 0 rows
#   (b) full valid column list, NO time constraint  -> 200 OK
# Together these show 45218 HAS reported to GLOS at some point (the query is
# valid, data exists somewhere in its history) but NOT in the last 6 days.
# What v2 could not say is WHEN it last reported, because fallback (b) just
# dumped a raw (and log-truncated) unbounded history response instead of
# asking for the one row that actually answers the question.
# v3 replaces that dump with ERDDAP's own `orderByMax("time")` feature -
# a server-side request for the single most recent row across the entire
# dataset, cheap and exact, then parses it and reports the real date and
# age instead of a truncated raw string.
#
...
def http_get(url, timeout=25):
    ...
    try:
        ...
        # ERDDAP writes its actual explanation into the response BODY, not
        # the HTTP reason phrase (which is often blank). Read it. This is
        # the exact gap that made v1's 404 unreadable.
        try:
...
def select_real_variables(das_vars):
    ...
    for name, meta in das_vars.items():
        ...
        if name.endswith("_fixed_depth") or std == "depth":
            # sea_water_velocity_N_depth is the depth companion for a CURRENT
            # sensor, not a water-TEMPERATURE sensor. It is real data, just
            # irrelevant to the water-temperature ladder question, and
            # including it doubled the column count on 45218's request for
            # no benefit. Route it to "other" instead.
            if name.startswith("sea_water_velocity_"):
...
def diagnostic_fallback_queries(ods, cols, start_iso, http_get_fn, now_utc):
    """When the primary combined CSV query fails, run two narrower queries to
    isolate the cause instead of guessing at it:
      (a) a MINIMAL query (time + just one real variable), same time window -
          if this also fails, the problem is not "too many columns."
      (b) ERDDAP's own orderByMax("time") - asks the server directly for the
          single most recent row in the ENTIRE dataset history, no time
          filter. Cheap (one row back, not a full history dump) and exact.
          If this succeeds, we get the real last-ever-reading date. If it
          ALSO fails, there is no data in this dataset at all, ever.
    Returns (results, last_ever) where results is a list of
    (description, status, detail) tuples for the report, and last_ever is
    either None or {"timestamp_utc": iso_str, "age_days": float}."""
    results = []
    last_ever = None
    if len(cols) > 1:
        minimal_cols = [cols[0], cols[1]]
        url = CSV_URL.format(ods=ods, cols=",".join(minimal_cols), start=start_iso)
        status, body, err = http_get_fn(url)
        detail = (body[:300] if body else err) or "(empty response)"
        results.append((f"(a) minimal query - time + {minimal_cols[1]} only, same window", status, detail))

    order_url = (
        f"https://seagull-erddap.glos.org/erddap/tabledap/obs_{ods}.csv?"
        f"{','.join(cols)}&orderByMax(%22time%22)"
    )
    status2, body2, err2 = http_get_fn(order_url)
    if status2 == 200 and body2:
        _, rows = parse_erddap_csv(body2)
        if rows and rows[0].get("time"):
            try:
                last_ts = parse_iso(rows[0]["time"])
                age_days = round((now_utc - last_ts).total_seconds() / 86400.0, 1)
                detail2 = f"most recent EVER reading for this dataset: {last_ts.isoformat()} ({age_days} days ago)"
                last_ever = {"timestamp_utc": last_ts.isoformat(), "age_days": age_days}
            except ValueError:
                detail2 = f"orderByMax succeeded but time column unparseable: {body2[:200]}"
        else:
            detail2 = f"orderByMax returned 200 but zero usable rows: {body2[:200]}"
    else:
        detail2 = (body2[:300] if body2 else err2) or "(empty response)"
    results.append(("(b) orderByMax(\"time\") - the single most recent row, ever, no time filter", status2, detail2))
    return results, last_ever
...
def analyze_target(target, geojson_obj, now_utc, http_get_fn=http_get):
    ...
    result = {
        ...
        "plausibility_warnings": [], "diagnostic_fallbacks": [],
    ...
    if csv_status != 200 or not csv_text:
        ...
        fallbacks, last_ever = diagnostic_fallback_queries(ods, cols, start_iso, http_get_fn, now_utc)
        result["diagnostic_fallbacks"] = fallbacks
        if last_ever:
            age_days = last_ever["age_days"]
            result["newest_reading_utc"] = last_ever["timestamp_utc"]
            result["age_hours"] = round(age_days * 24, 1)
            if age_days <= 0.25:
                result["verdict_live"] = "LIVE (found via orderByMax, not the recent window)"
            elif age_days <= 2:
                result["verdict_live"] = f"REPORTING BUT STALE - last reading {age_days} days ago"
            else:
                result["verdict_live"] = f"NOT LIVE - last-ever reading was {age_days} days ago, nothing since"
        else:
            result["verdict_live"] = "NOT LIVE - zero data found for this dataset at any time, ever"
        return result
    result["csv_ok"] = True
...
def print_target_report(target, result):
    ...
    if result.get("diagnostic_fallbacks"):
        print("DIAGNOSTIC FALLBACK QUERIES (primary query failed, narrowing down why):")
        for desc, status, detail in result["diagnostic_fallbacks"]:
            print(f"    {desc}")
