"""
PIERBITE — LMHOFS History & Jitter Probe (v2)
Generated: 2026-07-27
v2 fix: added the missing lmhofs_escape() call (percent-encodes [ ]
in the OPeNDAP query string). v1 omitted this and every node read
failed with HTTP 400 as a result — this is a bug in the PROBE only,
copied incompletely from fetch_data.py; production was never affected.
...
def lmhofs_escape(query):
    """Percent-escape array brackets. NOT optional — NOAA's OPeNDAP
    server returns HTTP 400 on raw [ ] characters in the query string.
    Copied from fetch_data.py v12 — this was missing from v1 of this
    probe, which is why v1's Q2/Q3 checks all failed with HTTP 400."""
