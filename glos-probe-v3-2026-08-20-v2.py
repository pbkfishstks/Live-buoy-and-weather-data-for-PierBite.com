#!/usr/bin/env python3
# PIERBITE | GLOS Probe v3 | 2026-08-20T19:10:00Z | v2
# Repo root filename (GitHub, prefix removed per D211): glos-probe-v3-2026-08-20-v2.py
# Local archive filename (Paul's machine):             Scripts_glos-probe-v3-2026-08-20-v2.py
#
# WHY v2 EXISTS: v1 ran clean (green check) but its PRIMARY question came back
# "ERDDAP .csv OK: False (error: HTTPError 404: )" - blank after the colon.
# Neshotah (598) and Port Washington (250) both returned real LIVE data with
# the same request shape, so the 404 is specific to 45218/obs_576 - but v1
# could not say WHY, because it never read the response BODY on an HTTPError,
# only the (usually empty) HTTP reason phrase. ERDDAP writes its actual error
# message into the body, not the reason phrase. v2 fixes exactly that, plus:
#   - auto-retries with a minimal query and with no time constraint at all
#     when the primary combined query fails, so the next run says definitively
#     whether 45218 has zero data ever, zero data recently, or a bad variable
#     name in the request - instead of guessing.
#   - stops requesting the 16 sea_water_velocity_*_depth columns, which are
#     irrelevant to the water-temperature question and were only in the
#     request because v1's classifier treated any standard_name=="depth"
#     variable as a water-temperature depth companion.
#
...
def diagnostic_fallback_queries(ods, cols, start_iso, http_get_fn):
    ...
    isolate the cause instead of guessing at it:
      ...
      (b) the FULL column list with NO time constraint at all (entire
          history) - if this succeeds, the station has data but none in the
          requested window (genuinely not reporting lately). If it ALSO
          fails, the problem is a bad variable name, not a reporting gap.
    Returns a list of (description, status, detail) tuples for the report."""
    results = []
    if len(cols) > 1:
    ...
    url_no_constraint = f"https://seagull-erddap.glos.org/erddap/tabledap/obs_{ods}.csv?{','.join(cols)}"
    status2, body2, err2 = http_get_fn(url_no_constraint)
    detail2 = (body2[:300] if body2 else err2) or "(empty response)"
    results.append(("(b) full column list, NO time constraint - entire history", status2, detail2))
    return results
...
def analyze_target(target, geojson_obj, now_utc, http_get_fn=http_get):
    ...
    if csv_status != 200 or not csv_text:
        ...
        result["diagnostic_fallbacks"] = diagnostic_fallback_queries(ods, cols, start_iso, http_get_fn)
        result["verdict_live"] = "INCONCLUSIVE - primary query failed, see diagnostic_fallbacks below"
        return result
    result["csv_ok"] = True
